...
use addons\kefu\library\pushapi\UniPush;
...
class Common
{
    ...
    public static function sessionList($csrId, $limit)
    {
        // 读取会话列表
        ...
        foreach ($session as $key => $value) {
            // 最后一条聊天记录
            $lastMessage = Db::name('kefu_record')
            ...
            $session[$key]['online']   = $value['wechat_openid'] ? 1 : Gateway::isUidOnline($value['session_user']);
            $session[$key]['avatar']   = $value['fu_avatar'] ? $value['fu_avatar'] : $value['avatar'];
            $session[$key]['nickname'] = $value['fu_nickname'] ? $value['fu_nickname'] . '(' . $value['nickname'] . ')' : $value['nickname'];
            $session[$key]['avatar']   = self::imgSrcFill($session[$key]['avatar'], true);

            // 用户发来的未读消息数
            $session[$key]['unread_msg_count'] = Db::name('kefu_record')
                ->where('session_id', $value['id'])
            ...
            unset($session[$key]['fu_avatar'], $session[$key]['fu_nickname']);
        }
    ...
    public static function checkAdmin($admin_token, $admin_id = 0)
    {
        ...
        if ($admin_id) {
            ...
            $admin = Db::name('admin')->field('id,avatar,nickname,username,token')->where('id', $admin_id)->find();
            if (!$admin) {
                return false;
            }
        } else {
            ...
            // ios 网络传输特殊符号兼容
            if (strstr($admin_token, '~') !== false) {
                $admin_token = str_replace('~', '|', $admin_token);
            }
            ...
            if ($id && $keeptime && $expiretime && $key && $expiretime > time()) {
                ...
                $admin = Db::name('admin')->field('id,avatar,nickname,username,token')->where('id', $id)->find();
    ...
    public static function chatRecord($session_id, $message, $message_type = 0, $sender = false, $message_id = false)
    {
        ...
        if ($sender_identity == 1) {
            // 过滤除了img和div的所有标签
            $message_html = strip_tags($message_html, "<img><div>");
        ...
        if (isset($session_user_info['wechat_openid']) && $session_user_info['wechat_openid']) {
            ...
            if ($message_type == 1) {
                ...
                $send_message_text = self::imgSrcFill($message_html);
            } else {
                $preg = '/<img.*?src="(.*?)".*?>/is';
        ...
        if (Db::name('kefu_record')->insert($message)) {
            ...
            if (class_exists('\GatewayWorker\Lib\Gateway') && $sender != $session_user) {
                ...
                if (Gateway::isUidOnline($session_user)) {
                    ...
                    // 用户不在线，推送
                    self::offlineMessagePush($session_user_info['id'], $session_user_info['source'], $message['session_id'], $sender_info['nickname'], self::formatMessage($message));
                }
            }
     ...
     * 离线消息推送-uniPush
     */
    public static function offlineMessagePush($recipientId, $recipientSource, $sessionId, $title, $content)
    {
        $sysUniPushSwitch = Db::name('kefu_config')->where('name', 'uni_push_switch')->value('value');
        if ($sysUniPushSwitch == 0) {
            return false;
        }

        $userCid = Db::name('kefu_user_push_clientid')
            ->field('clientid, platform')
            ->where('user_id', $recipientId)
            ->where('source', $recipientSource)
