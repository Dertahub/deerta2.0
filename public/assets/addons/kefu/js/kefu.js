...
var KeFu = {
    ...
    record_scroll_height: 0,// 轨迹记录窗口的滚动条高度
    unexpected_records: 0,// 翻页意外消息计数
    resize_load: 0,
    group_show: {
        'dialogue': false,
        'invitation': false,
    ...
    initialize: function (url = window.location.hostname, modulename = 'index', initSuccess = null, fixed_csr = 0) {

        KeFu.url = url;
        KeFu.query_csr = KeFu.getQueryVariable('csr')
        if (!KeFu.query_csr) {
    ...
    ConnectSocket: function () {
        ...
        if (window.location.protocol === 'https:' && parseInt(KeFu.config.wss_switch) === 0) {
            $('#kefu_error').html('https 下须创建 wss 服务才能连接网络，请参考文档!');
            KeFu.toggle_popover('show', 'https 下须创建 wss 服务才能连接网络，请参考文档!');
            return;
        }
    ...
    domsg: {
        ...
        initialize: (msg) => {
            ...
            if (msg.data.modulename == 'admin') {
                ...
                KeFu.changeSession(KeFu.session_id, 'session', false);
            } else {
                if (msg.data.new_msg) {
        ...
        chat_record: (msg) => {
            if (msg.data.page == 1) {
                $('#kefu_scroll').html('<div class="chat-records-grow"></div>');
            }
            ...
            for (let i in chat_record) {
                for (let y in chat_record[i].data) {
                    KeFu.buildRecord(chat_record[i].data[y])
                }

                if (msg.data.page == 1) {
                    KeFu.buildPrompt(chat_record[i].datetime);
                }
            }
                    ...
                    if (msg.data.session_user == KeFu.session_user && KeFu.window_is_show) {
                        ...
                        if (session.data('group') != 'dialogue') {
                            ...
                            if ($('#kefu_trajectory').css('display') != 'block') {
                                // 必要时，去除该会话的红点
                                KeFu.buildRecord(msg.data, 'prepend');
                                $('#kefu_scroll').scrollTop(0);
    ...
    buildPrompt: function (data, fun = 'append') {
        if (fun == 'append') {
            $('#kefu_scroll').append('<div class="status"><span>' + data + '</span></div>');
        } else {
            $('#kefu_scroll .chat-records-grow').after('<div class="status"><span>' + data + '</span></div>');
        }
    },
    ...
    changeSession: function (session_id, why = 'session', window_close_load_log = true) {
        ...
        if (element.data('group') == 'invitation' || why == 'trajectory') {
            ...
            KeFu.unexpected_records = 0;
        }

        if (KeFu.window_is_show) {
            KeFu.ws_send(load_log);
        } else if (window_close_load_log) {
            KeFu.window_show_event = function () {
                KeFu.ws_send(load_log);
            }
        }
    ...
    buildRecord: function (data, fun = 'append', message_id = 'none') {
        var message = '';
        ...
        if (fun == 'append') {
            ...
            $('#kefu_scroll .chat-records-grow').after('<div class="bubble ' + data.sender + '">' + message + status_html + '</div>');
        }
    },
    /**
     * 构建商品或订单卡片
    ...
    buildUrl: function (url, modulename, type = 'ws', wsport = 1818) {
        ...
        var buildFun = new Map([
            ...
            ['mobile', () => {
                let query = ''
                if (KeFu.query_csr) {
                    query = `?csr=${KeFu.query_csr}`
                }
                if (!KeFu.query_csr && KeFu.fixed_csr) {
                    query = `?fixed_csr=${KeFu.fixed_csr}`
                }
                return protocol + url + port + '/addons/kefu/index/mobile' + query;
            }],
    ...
    sendMessage: function (message, message_type) {
        ...
        KeFu.buildRecord(data, 'prepend', message_id);
    ...
    */
    eventReg: function () {
        ...
        // 加载更多聊天记录和轨迹
        document.addEventListener('scroll', KeFu.debounce(function (event) {

            if (KeFu.chat_record_page != 'done') {
                ...
                if (event.target.id == 'kefu_scroll' && scrollTop + clientHeight + 10 >= scrollHeight) {
                    ...
                    var load_message = {
                        ...
                        data: {
                            ...
                            unexpected_records: KeFu.unexpected_records,
                        }
                    };

                    KeFu.ws_send(load_message);
                } else if (scrollTop === 0 && event.target.id == 'kefu_trajectory') {
