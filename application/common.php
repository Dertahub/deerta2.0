...
if (!function_exists('over_time')) {
    /**
     * 超时取消
     * 使用时结合api.php的beginTime
     *
     * @param string $begin
     */
    function over_time(string $begin)
    {
        // 超过3s阻止提交
        if(\fast\Date::span(time(), $begin, 'seconds') > 3){
            exception(__('request timeout'));
        }
        return true;
    }
}
if (!function_exists('unique_order_sn')) {
    /**
     * 生成唯一订单号
     *
     * @param string $prefix
     ...
     * @return string
     */
    function unique_order_sn($prefix = '', $uid = 0): string
    {
        return $prefix . date("ymdhis") . sprintf("%08d", $uid) . mt_rand(1000, 9999);
    }
}
if (!function_exists('write_log')) {
     ...
     * @return bool
     */
    function write_log($msg, $path): bool
    {
        ...
        // 构建日志文件的完整路径
        $logDir = ROOT_PATH . 'runtime' . DS . $path;
        $logFile = $logDir . DS . date("Ymd") . '.log';
