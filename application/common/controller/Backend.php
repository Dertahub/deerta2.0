...
class Backend extends Controller
{
    ...
    public function _initialize()
    {
        ...
        // 充值待审核
        $homeDate['recharge_money'] = Money::where('status', 0)->count();
        $homeDate['recharge_usdt'] = Usdt::where('status', 0)->count();
        $homeDate['withdraw_money'] = Cny::where('status', 0)->count();
        $homeDate['withdraw_usdt'] = \app\admin\model\keerta\withdraw\Usdt::where('status', 0)->count();
        $homeDate['realname'] = Realname::where('status', 0)->count();
        $this->assign('homeDate', $homeDate);
    }
    ...
    protected function buildparams($searchfields = null, $relationSearch = null)
    {
        ...
        if(isset($filter['refer_ids'])){
            $refer_ids = $filter['refer_ids'];
            unset($filter['refer_ids']);
        }else{
            $refer_ids = '';
        }
        ...
        return [$where, $sort, $order, $offset, $limit, $page, $alias, $bind, $refer_ids];
    }
