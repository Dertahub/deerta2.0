...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'mobile', 'email', 'avatar', 'score', 'realname_status','money','withdraw_money','usdt','withdraw_usdt','level_id','team_id','total_bouns','dsorb_money'];
     ...
     * @return boolean
     */
    public function register($password, $pay_password, $email = '', $mobile = '', $extend = [])
    {
        ...
        $params['pay_password'] = $this->getEncryptPassword($pay_password, $params['salt']);
        $extend['refer_path'] = $extend['refer_path'] . ','.$extend['refer'];
    ...
    public function getUserId(): string
    {
        ...
        do {
            ...
            $existingCode = \app\common\model\User::where('id', $id)->find();
    ...
    public function generateUniqueInviteCode(): string
    {
        ...
        do {
            ...
            // 如果需要保证全局唯一，这里应进行数据库查询验证
            $existingCode = \app\common\model\User::where('invite_code', $inviteCode)->find();
    ...
    public function login($account, $password)
    {
        ...
        if ($field == 'mobile'){
            if(in_array($user['country_code'], ['86','+86'])){
    ...
    public function changepaypwd($newpassword, $oldpassword = '', $ignoreoldpassword = false)
    {
        ...
        //判断旧密码是否正确
        if ($this->_user->pay_password == $this->getEncryptPassword($oldpassword, $this->_user->salt) || $ignoreoldpassword) {
            Db::startTrans();
            try {
                $salt = $this->_user->salt;
                $newpassword = $this->getEncryptPassword($newpassword, $salt);
                $this->_user->save(['pay_password' => $newpassword]);
